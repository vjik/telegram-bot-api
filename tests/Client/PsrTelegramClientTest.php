...
final class PsrTelegramClientTest extends TestCase
{
    ...
    public function testPostWithDataAndNestedFiles(): void
    {
        $httpRequest = new Request();
        ...
        $httpClient
            ...
            ->with(
                new Callback(function ($request): bool {
                    ...
                    $requestHeaders = $request->getHeaders();
                    $this->assertSame(['Content-Length', 'Content-Type'], array_keys($requestHeaders));
                    $this->assertSame($requestHeaders['Content-Length'], ['411']);
                    $this->assertSame([0], array_keys($requestHeaders['Content-Type']));
                    $this->assertSame(
                        1,
                        preg_match(
                    ...
                    $this->assertStringContainsStringIgnoringLineEndings(
                        <<<TEXT
                            ...
                            {"title":"Face"}
                            --$matches[1]
                            Content-Disposition: form-data; name="photos[picture]"; filename="face.png"
                            Content-Length: 14
                            Content-Type: image/png
